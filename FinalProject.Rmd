...
#Graphic for monthly
In the monthly graphic we displayed how each of the eight select countries protiein movemnt per month changed in relation to other countries over time between September 2018 and September 2019. The layered boxplots show the average protein movement from September 2018 to September 2019. This allows us to compare the trend of the countries' monthly protein movement to their overall movement for the year. For example, this chart shows that Mexico's and Canada's monthly protein movement remained around the median protein movement from September 2018 to June 2019; however, from May 2019 onward, we can see that Canada's protein movement continued to increase while Mexcio's increased for two months and then declined. We can also see that the protein movement for countries such as China, Vietnam, and South Korea increased starting July 2019. 
...
LivestockMonthlyClean%>%
  filter( country %in% c("Canada","Mexico", "China (Mainland)", "Laos", "Phillipines","South Korea", "Japan", "Vietnam", "Australia", "New Zealand")) %>%
  ggplot(aes(x = Month, y = Protein_Movement)) +
  geom_point(size = 2) + aes(color = country) + geom_boxplot() + theme(axis.text.x = element_text(angle = 45))+
  ggtitle("Protein Movement from 2018 to September 2019 with Yearly Boxplots")
