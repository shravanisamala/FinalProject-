...
#Yearly Data
To clean up the yearly data for livestock, we renamed each variable so that it showed the amount of meat imported and exported in each year for each country. Then, we converted the data frame into narrow format by including all of the years in one big variable called year. This way, we can hopefully track the movement of meat imports and exports over the years for the countries that we would like to look at. 
```{r}
...
As seen in the data analysis above, it appears that Canada imports and exports the most meat out of all of the countries; Mexico also is the runner up from 2014 to 2018. However, after looking at the data for this year, the United Kingdom has taken over Canada's place in the rankings. In fact, Canada and Mexico are no longer in the top ten meat importers and exporters in 2019. As we expected, China, the Phillipines, and Vietnam moved up in the rankings; however, they are not the top meat importers and exporters. 
```{r}
...
To clean up the Monthly data on livestock, we only included the variables we wanted to work with: Country, Month, and Protein Movement (in pounds). This format allows us to see the meat import and export differences on a monthly basis for each country. 
```{r}
#Monthly
...
=======
  summarise(Protien_Movement = sum(total_two))%>%
  mutate(Month = ymd(Month,truncated = 1))
  
>>>>>>> 99566a6a1c7fe9028997a4728ab63b0554db4d00
...
# Graphic for year and month 2018 - 2019 data layered.  
```{r}
